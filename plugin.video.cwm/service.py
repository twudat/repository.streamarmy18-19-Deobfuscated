...
databases = translatePath(os.path.join(
...
try:
    c.executescript(
        "CREATE TABLE IF NOT EXISTS chaturbate (name, url, image);")
except:
    pass
conn.close()
...
while not monitor.abortRequested():
    if i <= 500:
        ...
        if len(e) < 1:
            dialog.notification(
                AddonTitle, '[COLOR pink]No Performers Added To Favs, Monitor is on though![/COLOR]', AddonIcon, 2500)
            quit()
        else:
            for (name, url, image) in e:
                if name not in Checked:
                    ...
                    if source:
                        dialog.notification(
                            AddonTitle, '[COLOR pink]Performer %s is now Online[/COLOR]' % name, image, 2500)
                        Checked.append(name)
                        xbmc.sleep(5000)
                    else:
                        pass
