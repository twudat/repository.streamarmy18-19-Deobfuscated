...
icon = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_, 'icon.png'))
fanarts = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_, 'fanart.jpg'))
#############################################################
...
Addon_Image = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_, 'icon.png'))
Background_Image = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'searchblack.png'))
...
def passed(self, title):
    ...
    newurl = ('%s?api_key=%s&append_to_response=external_ids' %
              (constructurl, TMDBAPI))
    ...
    for i in data:
        ...
        desc = i['overview']
        if desc == '':
            desc = 'No Synophsis Available'
        desc = airdate+desc
        if (currentdate < date):
            title = ('[COLOR deepskyblue][B]S%s E%s | %s[/B][/COLOR]' %
                     (seasonno, epino, name))
        else:
            title = ('S%s E%s | %s' % (seasonno, epino, name))
...
class list_window(pyxbmct.AddonFullWindow):
    ...
    def set_info_controls(self):
        self.Hello = pyxbmct.Label(
            '', textColor='0xFF00f72c', font='font60', alignment=pyxbmct.ALIGN_CENTER)
    ...
    def set_active_controls(self):
        ...
        self.List2 = pyxbmct.List(buttonFocusTexture=Listbg, _space=12,
                                  _itemTextYOffset=-7, _itemTextXOffset=-1, textColor='0xFFFFFFFF')
